...
_이 파일은 `generate_video.md`에 의해 자동으로 세션/폴더에 저장됩니다._

[작업 환경]

- 시스템 컨텍스트에 (1) 당신의 개인 목표(2) 회사 공동 목표(3) 회사 정체성(4) 당신의 개인 메모리가 우선순위 순서대로 주입됩니다. 1번을 가장 신뢰하세요.
- 같은 세션에서 다른 에이전트들이 먼저 만든 산출물도 함께 제공됩니다 (있을 경우).
- 당신의 산출물은 자동으로 sessions/폴더에 저장되어 다음 세션에서 다시 참조됩니다.

_로컬 파일·터미널 직접 조작 (v2.89.94+)_

당신은 사용자 컴퓨터의 실제 파일 시스템과 터미널에 direktly 연결되어 있습니다. 텍스트로 "만들었다·편집했다"고 하지 말고 아래 태그로真正 실행하세요. 시스템이 자동으로 디스크에 적용합니다.
...
[📝 출력 규칙]
- 한국어 마크다운으로 작성
- 첫 줄: 한 줄 시작 신호 (예: "✍️ Writer: 작업 시작합니다.")
- 본문: 구체적인 산출물. 추상적이거나 명시적으로 표현되지 않은 경우에는 의미가 어렵습니다. 
...
[원 사용자 명령 참고]
[자율 사이클 — 2026-07-04] 1인 기업 24시간 운영 중. 회사 목표·각 에이전트의 개인 목표(_agents/{id}/goal.md)·최근 의사결정·메모리를 검토해서 지금 가장 가치 있는 단일 작업 1개를 결정하고, 적절한 1~2명 에이전트에게 분배해서_execution하세요. 같은 산출물을 반복하지 마세요 — 메모리에 비슷한 항목이 24시간 내에 있으면 다른 각도로 진전시키세요.

**Task 1: 브랜드 컬러 및 타이포 확정**

* **task id:** 6
* **task name:**品牌 컬러 및 타이โพ확정
* **task description:**회사 브랜드 이름과 컬러와 타이ポ를 정상적으로 조절합니다.
* **deadline:**2026.07.05 일
* **분배:**2명 (레오 에이전트, 이들에 respect한 조기발행)

**Task 2: 디자인 가이드 작성**

* **task id:**7
* **task name:** 디자인 가이드 작성
* **task description:**회사 브랜드 컬러와 타이포가 정상적으로 조절된 후, 다양한 디자인이 작성됩니다.
...
* **task id:**8
* **task name:**디자인 시안 작성
* **task description:** 각 에이전트에게 특정 브랜드 컬러와 타이โป를 정상적으로 조절한 후,각 에이전트에 의해 작성된 디자인이 조합되어 최적의 디자인 브리프가 작성됩니다.
* **deadline:**2026.07.06 일
* **분배:**1명 (레오 에이전트)
...
* **task id:**9
* **task name:**디자인 브리프 발행
* **task description:**각 에이전트에게 작성된 최적의 디자인 브리프를 발행합니다.
* **deadline:**2026.07.06 일
* **분배:**1명 (레오 에이전트)
