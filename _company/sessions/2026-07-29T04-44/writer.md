...
**자율 사이클: 2026-07-29 12:00**



1.  **카피 라이팅 작성**
    *   `carpitalization.md` (이미지/아이콘, 텍스트/ikonography, 문법/lexicography) → 매력적인 카پی라이팅과 캡션을 제시합니다.
    *   `(제목`, `본문`, `마감)` 형식으로 작성되어, 핵심 메시지를 가장 강력하게 전달하는 데 집중합니다. `[제목]`은 1개 단어로 구성되어, `.md` 파일의 기본 이름을 사용하므로, 이 항목에 대한 텍스트를 포함하면, `.md` 파일이 automatically `.zip`ファイル로 확장됩니다.
    *   `본문`: 이 항목에는 본문과 핵심 메시지까지 포함되어야 합니다. `iconography`은 `icon`과 `iconography`의 합성으로, `(icon)`을 사용하여, `iconography`의 항목이 `icon`의 경우 1개 단어로 구성되어, `.md` 파일의 기본 이름을 사용하는 것을 주의해야 합니다.
    *   `[마감]`: 마감의 명칭을 포함하여, 이 항목에 대한 텍스트를 포함하면, `.md` 파일이 automatically `.zip`ファイル로 확장됩니다.



2.  **캡션 작성**
    *   `campaign.md` (이미지/아이콘, 텍스트/ikonography, 문법/lexicography) → 매력적인 캡션을 제시합니다.
    *   `(제목`, `본문`, `마감)` 형식으로 작성되어, 핵심 메시지를 가장 강력하게 전달하는 데 집중합니다. `[제목]`은 1개 단어로 구성되어, `.md` 파일의 기본 이름을 사용하므로, 이 항목에 대한 텍스트를 포함하면, `.md` 파일이 automatically `.zip`ファイル로 확장됩니다.



3.  **원 사용자 명령**
    *   `goal.md` (시각적 주목도 높여주고 클릭을 유도하는 매력적인 마emo리) → 매력적인 카피 라이팅과 캡션을 제시합니다.
    *   `(제목`, `본문`, `마감)` 형식으로 작성되어, 핵심 메시지를 가장 강력하게 전달하는 데 집중합니다. `[제목]`은 1개 단어로 구성되어, `.md` 파일의 기본 이름을 사용하므로, 이 항목에 대한 텍스트를 포함하면, `.md` 파일이 automatically `.zip`ファイル로 확장됩니다.



[전문 영역]
카피 라이팅 및 캡션 작성, 원 sử자 명령, 자율 사이클, 회사 목표, 각 에이전트의 개인 목표, 최_recent 의사결정, 메모리를 검토



[작업 환경]
- 시스템 컨텍스트에 (1) 당신의 개인 목표 (2) 회사 공동 목표 (3) 회사 정체성/의사결정 (4) 당신의 개인 메모리가 우선순위 순서대로 주입됩니다. 1번을 가장 신뢰하세요.
- 같은 세션에서 다른 에이전트들이 먼저 만든 산출물도 함께 제공됩니다 (있은 경우)
- 당신의 산출물은 자동으로 sessions/ 폴더에 저장되어 다음 세션에서 다시 참조됩니다.



[로컬 파일·터미널 직접 조작]
당신은 사용자 컴퓨터의 실제文件 시스템과 터미널에 직접 연결되어 있습니다. 텍스트로 "만들었다·편집했다"고 하지 말고 아래 태그로 realmente 실행하는 것을 고려합니다. hệ산점이 자동으로 디스크에 적용됩니다.
...
📄 generate_card_images.py
     1: import os
     2: from PIL import Image, ImageDraw, ImageFont, ImageColor
     4: def generate_card_image(title, subtitle, output_filename, logo_path, font_path):
     5:     # 이미지 설정
     6:     width, height = 1080, 1080  # 인스타그램 표준 크기
     7:     main_bg_color = ImageColor.getrgb("#4A67A3") # 메인 배경색
     8:     accent_yellow = ImageColor.getrgb("#FFD700") # Accent Yellow
    10:     # 이미지 생성
    11:     img = Image.new('RGB', (width, height), color = main_bg_color)
