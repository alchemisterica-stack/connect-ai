# ✍️ Writer — 2026-06-20 10:00에 비해 가장 가치 있는 단일 작업 선택

CEO의 지시에 따라 가장 가치 있는 단일 작업을 선택하고, 적절한 1~2명 에이전트에게 분배해서_execution하는 방법을 다음과 같이 작성합니다.

### 🌞 2026-06-20] 24시간 업무가 켜져 있으면 이 미션을 향해 자동으로 한 스텝씩 일합니다.

[작업 환경]
- 시스템 컨텍스트에 (1) 당신의 개인 목표 (_agents/{id}/goal.md)과 회사 공동 목표(_company/mission/guideline.md)와 함께 작업할 수 있습니다.
- 같은 세션에서 다른 에이전트들이 먼저 만든 산출물도 함께 제공됩니다 (있을 경우).

[로컬 파일·터미널 직접 조작 (v2.89.94+)]
- 사용자 컴퓨터의 실제 파일 시스템과 터미널에 직접 연결되어 있습니다. 텍스트로 "만들었다·편elist"고 하지 말고 아래 태그로 realmente execute 합니다. 시스템이 자동으로 디스크에 적용합니다.
  ...
  • <read_file path="..."/> — 32KB까지 읽기 (cat -n 줄번호 포함). 편집 전enet 반드시 먼저 read
...
- 당신의 도구 폴더 (_agents/{id}/tools/_) 와 사용자 프로젝트 폴더는 다릅니다. 사용자가 "이 프로젝트에 ..."라고 했으면 그 폴더는 도구 폴더 안이 아닙니다.

[로컬 파일·터미널 직접 조작 (v2.89.94+)]
- 당신은 사용자 컴퓨터의 실제 파일 시스템과 터미널에 direktly connected nên `create_file`와 `edit_file` etc. commands가 시스템에 자동으로 적용됩니다.
- `.gitignore`ファイル을 수정할 수 있습니다. 예를 들어, `*.pyc`는 기존의 .gitignore 안에 포함되어 있지 않습니다.
...
_CEO의 지시에 따라 가장 가치 있는 단일 작업을 선택하고, 적절한 1~2명 에이전트에게 분배해서execution하는 방법을 다음과 같이 작성합니다._

- **자율 사이클** (_2026-06-19) 1인 기업 24시간 운영 중. 회사 목표·각 에이전트의 개인 목표(_agents/{id}/goal.md)과 최근 의사결정·메모리를 검토해서 지금 가장 가치 있는 단일 작업 1개를 결정하고, 적절한 1~2명 에이전트에게 분배해서Execution하는 방법은 다음과 같습니다._

- **인스타그램**(_instagram_api.py) 
  - 가장 가치 있는 단일 작업은 인stagram입니다. 인stagram의 트렌드, 성공적인 카드 뉴스 형식, 시각적 스타일 및 사용자 참여 패턴을 분석할 수 있으며, 매번 도구마다 같은 키를 넣지 않아도 돼요.
...
- **자율 사이클** (_2026-06-19) 1인 기업 24시간 운영 중. 회사 목표·각 에이전트의 개인 목표(_agents/{id}/goal.md)과 최근 의사결정·메모리를 검토해서 지금 가장 가치 있는 단일 작업 1개를 결정하고, 적절한 1~2명 에이전트에게 분배서Execution하는 방법은 다음과 같습니다._

- **인스타그램**(_instagram_api.py) 
  - 가장 가치 있는 단일 작업은 인stagram입니다. 인stagram의 트렌드, 성공적인 카드 뉴스 형식, 시각적 스타일 및 사용자 참여 패턴을 analyzing 하며, 매번 도구마다 같은 키를 넣지 않아도 돼요.

- **인스타그램**(_instagram_api.py) 
  - 가장 적합한 에이전트는 `@instagram_ae`입니다. 이 에이전트는 인stagram 채널에 대한 chuyên sâu의Knowledge를 가지고 있으며, 최근 30일 떡상 영상을 수집하고, 로컬 LLM(Ollama/LM Studio)로 패턴을 analyzing 해 다음 영상 기획안(제목·썸네일·후크)을 도출하는 데 도움이 됩니다.
...
📄 scratch/copy_to_brain.py
     1: import shutil
     2: import os
     4: SRC_RAW = r"C:\Users\User\my-ai-office\assets\custom_recipe_photos\temp_downloads\20260616_150650.jpg"
     5: SRC_REF = r"C:\Users\User\my-ai-office\assets\custom_recipe_photos\temp_downloads\ref_150650\20260616_150650(1).jpg"
     7: DST_DIR = r"C:\Users\User\.gemini\antigravity\brain\fbbf98b4-a801-4d2f-b58a-404b9d3b1ef5"
     9: def main():
    10:     os.makedirs(DST_DIR, exist_ok=True)
    11:     
    12:     dst_raw = os.path.join(DST_DIR, "150650_raw.jpg")
    13:     dst_ref = os.path.join(DST_DIR, "150650_ref.jpg")
📄 scratch/diff_js.py
     1: import difflib
